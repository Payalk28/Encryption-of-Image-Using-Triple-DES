...
class Toplevel1:
    def decrypt(self):
        ...
        if len(self.Text1.get("1.0", "end-1c")) == 0:
            ...
            # decryption process
            cipherkey = DES3.new(key, DES3.MODE_CFB)
            ...
            if res.hexdigest() != str(hashcode.decode()):
                ...
                f.write(plaintext)
                f.close()
                msg = "The File has been decryted successfully.\nThe integrity check of the file has passed. The file is at the location:"
                result = tk.messagebox.showinfo("Decryption successful", msg)
                # if result == 'yes':
    ...
    def browse(self):
        filename = filedialog.askopenfilename(parent=root, initialdir="/", title="Select a File", filetypes=(
            ("Image files", "*.jpg"), ("Image files", "*png"), ("Image files", "*jpeg")))
